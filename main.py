...
def main():
    ...
    if st.session_state.uploaded_file is None:
        ...
        with col2:
            if st.button("♻️ Reset", key="reset_button", use_container_width=True):
    ...
    if len(st.session_state.csv_ops.table_data) > 0 and st.session_state.uploaded_file is not None:
        ...
        tab = st.radio("Select an action", options=["➕ Add Row", "✏️ Modify Existing Data"])
        ...
        elif tab == "✏️ Modify Existing Data":
            row_options = [
